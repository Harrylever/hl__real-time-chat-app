...
const {
  appMailClientId, appMailClientSecret, appMailRefreshToken, appMailClientEmail,
...
async function createTransporter() {
  try {
    const oauth2Client = new OAuth2(appMailClientId, appMailClientSecret, googleRedirectUri);

    oauth2Client.setCredentials({
      refresh_token: appMailRefreshToken,
    });
    ...
    const transporter = nodemailer.createTransport({
      ...
      auth: {
        type: 'OAuth2',
        user: appMailClientEmail,
        accessToken,
        clientId: appMailClientId,
        clientSecret: appMailClientSecret,
        refreshToken: appMailRefreshToken,
      },
    });
    return transporter;
  } catch (err) {
