  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10 xl:gap-16">
      {ourFeaturesList.map((tab, index) => (
        <FeatureTab key={index} prop={tab} index={index} />
      ))}
    </div>
  );
}

const FeatureTab = ({prop: { icon, title, context }, index}: {prop: IFeature, index: number}) => {
  return (
    <div className={clsx(['flex flex-col items-center justify-start gap-6 max-w-[390px] pb-5', {
      'border-b sm:border-none border-[#45454533] shadow-sm': index === 0 || index === 1
    }])}>
